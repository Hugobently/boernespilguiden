...
### What is Børnespilguiden?

Børnespilguiden (Danish for "Children's Game Guide") is a comprehensive Danish website that helps parents find age-appropriate games, apps, and streaming content for children aged 0-10 years.
...
│   │   │   ├── enhance-media/        # AI enhancement trigger
│   │   │   ├── import/               # Data import
│   │   │   ├── init-db/              # Database initialization
│   │   │   ├── migrate-media/        # Media migration
│   │   │   └── reimport-tmdb/        # TMDB reimport
│   │   ├── analytics/                # Event tracking
│   │   ├── boardgames/               # Board game CRUD
│   │   ├── games/                    # Digital game CRUD
│   │   ├── search/                   # Search with suggestions
│   │   ├── set-locale/               # Language preference
│   │   └── cron/daily-update/        # Scheduled jobs
...
│       │   └── kategori/[age]/       # Age category pages
│       ├── braetspil/                # Board games section
│       ├── film-serier/              # Movies & TV section
...
│   │   ├── Card.tsx, Input.tsx
│   │   ├── Rating.tsx, AgeIndicator.tsx
│   │   └── index.ts
│   ├── layout/                       # Layout components
│   │   ├── Header.tsx (510 lines)    # Main navigation
│   │   ├── Footer.tsx
│   │   ├── Navigation.tsx
...
│   │   ├── AgeFilter.tsx             # Age group selector
│   │   ├── CategoryFilter.tsx
│   │   ├── SearchBar.tsx
│   │   └── ParentFilters.tsx
...
│   ├── cache.ts (310 lines)          # Caching layer
│   ├── translations.ts               # Translation helpers
│   ├── analytics.ts                  # Analytics tracking
│   ├── search.ts                     # Search utilities
│   ├── seo.tsx                       # SEO helpers
│   ├── config/
│   │   └── age-groups.ts             # Age group configuration
│   ├── constants/
│   │   └── blacklist.ts              # Content blacklist
│   └── services/                     # External services
│       ├── ai-enhance.ts             # Media AI enhancement
│       ├── ai-enhance-game.ts        # Game AI enhancement
│       ├── tmdb.ts                   # TMDB API client
│       ├── tmdb-import.ts            # TMDB data import
│       └── dr-import.ts              # DR (Danish Radio) import
│
├── prisma/
...
```
┌─────────────┐     ┌─────────────────┐     ┌───────────────┐
│    Game     │     │   BoardGame     │     │     Media     │
├─────────────┤     ├─────────────────┤     ├───────────────┤
│ id          │     │ id              │     │ id            │
│ slug*       │     │ slug*           │     │ slug*         │
│ title       │     │ title           │     │ tmdbId*       │
│ description │     │ description     │     │ drEntityId*   │
│ minAge      │     │ minAge          │     │ title         │
│ maxAge      │     │ maxAge          │     │ type          │
│ platforms[] │     │ minPlayers      │     │ posterUrl     │
│ iconUrl     │     │ maxPlayers      │     │ ageMin        │
│ rating      │     │ playTime        │     │ ageMax        │
│ parentInfo  │     │ complexity      │     │ parentInfo    │
│ pros[]      │     │ imageUrl        │     │ pros[]        │
│ cons[]      │     │ rating          │     │ cons[]        │
│ ...         │     │ ...             │     │ ...           │
└─────────────┘     └─────────────────┘     └───────┬───────┘
      ...
      ▼                                             ▼
┌─────────────────┐                     ┌───────────────────┐
│ GameTranslation │                     │  StreamingInfo    │
├─────────────────┤                     ├───────────────────┤
│ id              │                     │ id                │
│ gameId (FK)     │                     │ mediaId (FK)      │
│ locale          │                     │ provider          │
│ title           │                     │ available         │
│ description     │                     │ isFree            │
│ ...             │                     │ url               │
...
model Game {
  ...
  parentInfo            String?   @db.Text  // "Hvad forældre skal vide"
...
export const ageGroups = [
  { slug: '0-3', label: '0-3 år', emoji: '👶', color: { bg: '#FFD1DC', text: '#8B4563' } },
  { slug: '3-6', label: '3-6 år', emoji: '🧒', color: { bg: '#BAFFC9', text: '#2D6A4F' } },
  { slug: '7+',  label: '7+ år',  emoji: '👦', color: { bg: '#BAE1FF', text: '#1D4E89' } },
];
```
...
| Service | URL | Purpose |
|---------|-----|---------|
| Prisma | Vercel Dashboard → Environment Variables | Database connection |
