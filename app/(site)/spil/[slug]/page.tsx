...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const game = await prisma.game.findFirst({
    where: { slug: params.slug, type: 'DIGITAL' },
  });

  if (!game) return { title: 'Spil ikke fundet' };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        {/* Image Section */}
        <div className="relative aspect-square rounded-3xl overflow-hidden bg-gradient-to-br from-sky to-lavender">
          {game.imageUrl ? (
            <img
              src={game.imageUrl}
              alt={game.title}
        ...
        <div>
          <div className="flex flex-wrap gap-2 mb-4">
            <Badge variant="blue">🎮 Digitalt Spil</Badge>
            ...
            <dl className="grid grid-cols-2 gap-4">
              ...
              {game.playTime && (
                <div>
                  ...
                  <dd className="font-semibold text-charcoal">{game.playTime}</dd>
                </div>
              )}
            </dl>
          </div>
